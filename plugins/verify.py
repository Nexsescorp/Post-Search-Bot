...
async def verify_(bot, update):
    ...
    if update.data.split("_")[1]=="approve":
       ...
       reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎪  JOIN OUR CHANNEL  🎪", url="https://t.me/Nexses_Corporation")]]))
       await update.message.edit(update.message.text.html.replace("#NewRequest", "#Approved"))
    else:
       await delete_group(id)
       await bot.send_message(chat_id=user, text=f"Your verification request for {name} has been declined 😐 This Bot Is Paid Please Contact Admin")
